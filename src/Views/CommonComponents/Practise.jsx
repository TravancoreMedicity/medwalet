...
import { getAllSlotMaster } from '../../Views/CommonComponents/useQueryFunctions';
...
const InputFileUpload = lazy(() => import('../../Pages/Parking/ParkingFileupload'))
const LabourSelectBox = lazy(() => import("../../Components/AutoComplete"))


export default function ValletForm({ refetch, allvehicles }) {
    ...
    const validateForm = useCallback(() => {
        ...
        if (selectedFile.length === 0) {
            warningNofity("Please upload vehicle images");
            hasError = true;
        }
    ...
    //image compression
    const HandleImageCompression = useCallback(async (files) => {
        const options = {
            maxSizeMB: 2,
        ...
        <Box sx={{ width: '100%', minHeight: 200, display: "flex", alignItems: 'center', justifyContent: 'center' }}>
            ...
            <Box sx={{ width: { xs: '100%', sm: '100%' }, height: '98%', display: 'flex', alignItems: 'center', flexDirection: 'column', bgcolor: 'white', borderRadius: 2 }}>
                <Box sx={{ width: '100%', minHeight: 200 }}>
                    ...
                    <Button  disabled={isLoading}  variant="contained" color="success" onClick={handlesubmit} sx={{ width: '100%' }}>
  ...
  //   // MappingData?.map((entry) => {
  //   //   const { driver_id, atendnace_time, attendnace_status, em_name } = entry;
  //   //   if (attendnace_status === "I") {
  //   //     // Store Check-In in the object
  //   //     attendanceObject[driver_id] = { em_name, CheckIn: atendnace_time };
  //   //   } else if (attendnace_status === "O" && attendanceObject[driver_id]) {
  //   //     // If CheckIn exists, pair it with CheckOut and remove from the object and push to the array
  //   //     const { em_name, CheckIn } = attendanceObject[driver_id];
  //   //     formattedAttendance.push({
  //   //       slNo: formattedAttendance.length + 1,
  //   //       DriverName: em_name,
  //   //       CheckIn,
  //   //       Checkout: atendnace_time,
  //   //     });
  //   //     delete attendanceObject[driver_id];  // Remove the paired entry
  //   //   }
  //   // });

  //   // Add any remaining unmatched Check-Ins as "Not Checked Out"
  //   Object.entries(attendanceObject).forEach(([driver_id, { em_name, CheckIn }]) => {
  //     formattedAttendance.push({
  //       slNo: formattedAttendance.length + 1,
  //       DriverName: em_name,
  //       CheckIn,
  ...
  // }, [selectedValue, todayvehicle, vehiclebetweendate, AttendaceReport, employeeDetail])